...
function ensureJavaJar(egg, dir, version, rt) {
  let jar = detectJar(dir);
  if (jar) return jar;
  const ver = version || "1.21.4";
  const dest = path.join(dir, egg.id === "paper" || egg.id === "vanilla-java" ? "server.jar" : "server.jar");
  pushLog(rt, `[HoptoNode] ${egg.id} ${ver} JAR indiriliyor…`, "system");
  if (egg.id === "fabric") {
    const url = `https://meta.fabricmc.net/v2/versions/loader/${encodeURIComponent(ver)}/0.16.10/1.0.1/server/jar`;
    curlFile(url, path.join(dir, "fabric-server-launch.jar"));
    return "fabric-server-launch.jar";
  }
  if (egg.id === "paper" || egg.id === "vanilla-java") {
    try {
      const meta = JSON.parse(curlText(`https://api.papermc.io/v2/projects/paper/versions/${encodeURIComponent(ver)}`));
      const build = (meta.builds || []).slice(-1)[0];
      if (!build) throw new Error("paper build yok");
      const name = `paper-${ver}-${build}.jar`;
      const url = `https://api.papermc.io/v2/projects/paper/versions/${encodeURIComponent(ver)}/builds/${build}/downloads/${name}`;
      curlFile(url, dest);
      return "server.jar";
    } catch (err) {
      pushLog(rt, `[HoptoNode] Paper inemedi (${err.message}), vanilla deneniyor`, "system");
      const man = JSON.parse(curlText("https://piston-meta.mojang.com/mc/game/version_manifest_v2.json"));
      const hit = (man.versions || []).find((v) => v.id === ver) || (man.versions || []).find((v) => v.id === "1.21.4");
      if (!hit) throw new Error("vanilla sürüm yok");
      const det = JSON.parse(curlText(hit.url));
      const jarUrl = det.downloads && det.downloads.server && det.downloads.server.url;
      if (!jarUrl) throw new Error("vanilla jar yok");
...
function startServer(server) {
  ...
  if (jar && commandExists("java")) {
    ...
    extraEnv = {};
    engine = "java";
    pushLog(rt, `[HoptoNode] Java ${jar} · heap ${heap} MB`, "system");
