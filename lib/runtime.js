...
function startServer(server) {
  ...
  const phpPhar = fs.existsSync(path.join(dir, "PocketMine-MP.phar"));
  ...
  let extraEnv = {
    ...
    HN_VERSION: String(vars.MC_VERSION || vars.BDX_VERSION || vars.PM_VERSION || "1.21.4"),
    HN_EGG: egg.id,
    HN_DIR: dir,
  };
  ...
  if (jar && commandExists("java")) {
    ...
    extraEnv = {};
    engine = "java";
  } else if (phpPhar && commandExists("php")) {
    cmd = "php";
    args = ["PocketMine-MP.phar", "--no-wizard", "--disable-ansi"];
    extraEnv = {};
    engine = "php";
  } else if (bedrockBin) {
