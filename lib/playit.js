...
let accountKey = String(process.env.PLAYIT_API_KEY || "").trim();
...
function setAccountKey(key) {
  accountKey = String(key || process.env.PLAYIT_API_KEY || "").trim();
}
...
function formatCreated(json) {
  const node = (json && json.data) || json || {};
  const tunnel = node.tunnel || node;
  applyTunnels([tunnel, ...state.tunnels.map((t) => ({ ...t, tunnel_type: t.type, origin: { data: { local_port: t.localPort } } }))]);
  return formatTunnel(tunnel) || formatTunnel(node);
}

async function createTunnel({ name, protocol, localPort }) {
  const bedrock = protocol === "bedrock";
  const body = {
    name: String(name || "hoptinode").slice(0, 32),
    tunnel_type: bedrock ? "minecraft-bedrock" : "minecraft-java",
    port_type: bedrock ? "udp" : "tcp",
    port_count: 1,
    origin: { type: "default", data: { local_ip: "127.0.0.1", local_port: Number(localPort) } },
    enabled: true,
  };
  ...
  if (created && created.status < 400) {
    ...
    const addr =
      formatTunnel((created.json && created.json.data) || created.json) ||
      addressFor({ port: localPort }, protocol);
    return { ok: true, address: addr, raw: created.json };
  }
  const msg =
    (created && created.json && created.json.data && created.json.data.message) ||
    `playit tünel ${created && created.status}`;
  const existing = state.tunnels.find((t) => t.kind === (bedrock ? "bedrock" : "java") && t.localPort === Number(localPort));
  if (existing && existing.address) return { ok: true, address: existing.address, reused: true };
...
function addressFor(server, protocol) {
  ...
  const port = server && Number(server.port);
  if (port) {
    const want = proto === "bedrock" ? "bedrock" : "java";
    const hit = state.tunnels.find((t) => Number(t.localPort) === port && t.kind === want && t.address);
    if (hit) return hit.address;
  }
  if (server && server.joinAddress) return server.joinAddress;
